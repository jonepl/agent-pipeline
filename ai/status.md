...
- Push credentials fix — plan-draft-prompt.md gains STEP 0 that sets git remote URL to https://x-access-token:${GH_TOKEN}@github.com/jonepl/agent-pipeline.git before any git operations; ghOpenPr now checks gh api repos/jonepl/agent-pipeline/branches/{branch} before gh pr create and throws a descriptive error if the branch is absent (agent push failed); worktree issue-13 planDrafter.test.ts updated to include mockOpenPr; 158 tests pass.
